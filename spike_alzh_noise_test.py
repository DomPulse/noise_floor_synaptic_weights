...
for index, n in enumerate(noise_levels):
	for epoch in range(num_epochs):  # loop over the dataset multiple times
		with torch.no_grad():
			...
			net[2].weight[:,:] = torch.from_numpy(np.add(virgin_second_layer, noise_2)[:,:])
			net[4].weight[:,:] = torch.from_numpy(np.add(virgin_third_layer, noise_3)[:,:])
			net[6].weight[:,:] = torch.from_numpy(np.add(virgin_fourth_layer, noise_4)[:,:])
