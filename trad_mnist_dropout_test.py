...
for index, n in enumerate(drop_levels):
	for epoch in range(num_epochs):  # loop over the dataset multiple times
		with torch.no_grad():
			...
			net[4].weight[:,:] = torch.from_numpy(np.multiply(virgin_second_layer, drop_2)[:,:])
			net[9].weight[:,:] = torch.from_numpy(np.multiply(virgin_third_layer, drop_3)[:,:])
			net[12].weight[:,:] = torch.from_numpy(np.multiply(virgin_fourth_layer, drop_4)[:,:])
...
plt.ylabel('accuracy')
plt.title('trad model dropout test')
plt.show()
